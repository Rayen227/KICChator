...
import { Live2DCubismFramework as live2dcubismframework } from "../CubismSdkForWeb/Framework/live2dcubismframework";
import { Live2DCubismFramework as cubismid } from "../CubismSdkForWeb/Framework/id/cubismid";
import { Live2DCubismFramework as cubismusermodel } from "../CubismSdkForWeb/Framework/model/cubismusermodel";
import { Live2DCubismFramework as icubismmodelsetting } from "../CubismSdkForWeb/Framework/icubismmodelsetting";
import { Live2DCubismFramework as cubismmodelsettingjson } from "../CubismSdkForWeb/Framework/cubismmodelsettingjson";
import { Live2DCubismFramework as cubismdefaultparameterid } from "../CubismSdkForWeb/Framework/cubismdefaultparameterid";
import { Live2DCubismFramework as acubismmotion } from "../CubismSdkForWeb/Framework/motion/acubismmotion";
import { Live2DCubismFramework as cubismeyeblink } from "../CubismSdkForWeb/Framework/effect/cubismeyeblink";
import { Live2DCubismFramework as cubismbreath } from "../CubismSdkForWeb/Framework/effect/cubismbreath";
import { Live2DCubismFramework as csmvector } from "../CubismSdkForWeb/Framework/type/csmvector";
import { Live2DCubismFramework as csmmap } from "../CubismSdkForWeb/Framework/type/csmmap";
import { Live2DCubismFramework as cubismmatrix44 } from "../CubismSdkForWeb/Framework/math/cubismmatrix44";
import { Live2DCubismFramework as cubismstring } from "../CubismSdkForWeb/Framework/utils/cubismstring";
import { Live2DCubismFramework as cubismmotion } from "../CubismSdkForWeb/Framework/motion/cubismmotion";
import { Live2DCubismFramework as cubismmotionqueuemanager } from "../CubismSdkForWeb/Framework/motion/cubismmotionqueuemanager";
import { Live2DCubismFramework as csmstring } from "../CubismSdkForWeb/Framework/type/csmstring";
import { Live2DCubismFramework as csmrect } from "../CubismSdkForWeb/Framework/type/csmrectf";
import { CubismLogInfo } from "../CubismSdkForWeb/Framework/utils/cubismdebug";
...
function deleteBuffer(buffer: ArrayBuffer, path: string = "") {
    Live2dPlatform.releaseBytes(buffer);
}
...
export default class Live2dModel extends CubismUserModel {
    ...
    public loadAssets(dir: string, fileName: string): void {
        ...
        Live2dPlatform.loadArrayBuffer(path)
            .then((arrayBuffer: ArrayBuffer) => {
                ...
                // ステートを更新
                self._state = LoadStep.LoadModel;
    ...
    private setupModel(setting: ICubismModelSetting): void {
        ...
        if (this._modelSetting.getModelFileName() != "") {
            ...
            Live2dPlatform.loadArrayBuffer(path).then((arrayBuffer) => {
                buffer = arrayBuffer;
                self.loadModel(buffer);
                deleteBuffer(buffer, path);
                self._state = LoadStep.LoadExpression;
            ...
            Live2dPlatform.printLog("Model data does not exist.");
        }
    ...
    private setupTextures(): void {
        ...
        if (this._state == LoadStep.LoadTexture) {
            ...
            for (let modelTextureNumber = 0; modelTextureNumber < textureCount; modelTextureNumber++) {
                ...
                // 読み込み
                this._app.textureManager.createTextureFromPngFile(texturePath, usePremultiply, onLoad);
    ...
    public update(): void {
        ...
        const deltaTimeSeconds: number = Live2dPlatform.getDeltaTime();
    ...
    public startMotion(group: string, no: number, priority: number): CubismMotionQueueEntryHandle {
        if (priority == Live2dDefine.PriorityForce) {
            ...
            if (this._debugMode) {
                Live2dPlatform.printLog("[APP]can't start motion.");
            }
        ...
        if (this._debugMode) {
            Live2dPlatform.printLog("[APP]start motion: [{0}_{1}", group, no);
        }
    ...
    public setExpression(expressionId: string): void {
        ...
        if (this._debugMode) {
            Live2dPlatform.printLog("[APP]expression: [{0}]", expressionId);
        }

        if (motion != null) {
            this._expressionManager.startMotionPriority(motion, false, Live2dDefine.PriorityForce);
        }
        else {
            if (this._debugMode) {
                Live2dPlatform.printLog("[APP]expression[{0}] is null", expressionId);
            }
        }
    }
    ...
    public preLoadMotionGroup(group: string): void {
        ...
        for (let i: number = 0; i < this._modelSetting.getMotionCount(group); i++) {
            ...
            if (this._debugMode) {
                Live2dPlatform.printLog("[APP]load motion: {0} => [{1}_{2}]", path, group, i);
            }
    ...
    public doDraw(): void {
        ...
        let scale = this._app.scale
        // キャンバスサイズを渡す
        let viewport: number[] = [
            -this._app.width * (scale / 2 - 0.5),
            -this._app.height * (scale / 2 - 0.5),
            this._app.width * scale,
            this._app.height * scale
        ];

        this.getRenderer().setRenderState(this._app.frameBuffer, viewport);
